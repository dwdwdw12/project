...
public class JoinController {
	...
	public int userPwdDuplicateCheck(@RequestParam("pwd") String pwd, @RequestParam("pwd_check") String pwd_check) {
		...
		if (pwd == pwd_check) {
			userPwdCnt = 1;
			log.info("userPwdCnt >> " + userPwdCnt);
		} else {
			userPwdCnt = 0;
			log.info("userPwdCnt >> " + userPwdCnt);
		}
	...
	@ResponseBody // json 값을 가져오므로 붙임(@requestparam으로 받아와서 userId로 저장)
	public String memberInfo(RedirectAttributes attr, KakaoUserVO vo, String phone_first, String phone_middle,
			String phone_last, String email, String mail_Domain, String addressDefault, String addressDetail) {
