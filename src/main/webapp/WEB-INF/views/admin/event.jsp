		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12 mx-auto tm-about-text-wrap text-center">
					<h2 class="text-uppercase mb-4">이벤트 게시판</h2>
				</div>
			</div>
				...
				<table class="table table-hover">
					...
					<c:forEach items="${vo}" var="list">
						<tbody>
							<tr class="flight">
								<td>${list.boardNum}</td>
								<td><a href="/boardEvent/view?boardNum=${list.boardNum}">${list.boardTitle}</a></td>
								<td>${list.startDate}</td>
								<td>${list.endDate}</td>
								<td>${list.readCount}</td>
								<td><button type="button" class="btn btn-primary btn-sm btn-block btn-custom" onclick="removeEvent('${list.boardNum}')">삭제</button></td>
							</tr>
						</tbody>
					</c:forEach>
				</table>
