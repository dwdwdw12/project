<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
<div style="height: 200px; overflow: auto; border-top: 1px solid #666;; background-color: #f7f7f7" >
<div class="terms_cont" name="terms07" tabindex="0">
							<p> 카카오항공에서는 항공여행시스템 제공 업체인 AMADEUS사에 당사의 예약 및 운송 시스템 운영을 위탁하고 있으며, 예약에 필요한 개인정보 국외 이전 사항은 다음과 같습니다. </p>
<p>&nbsp;</p>
<p>가. 개인정보를 이전받는 자</p>
<p>- AMADEUS IT GROUP,S.A (dataprotection@amadeus.com)</p>
<p>&nbsp;</p>
<p>나. 이전하는 개인정보 항목</p>
<p>- 항공권 예약시 : 카카오클럽 회원번호, 탑승자 성명, 생년월일, 성별, 연락처, 이메일</p>
<p>- 미국 출/도착 항공권 예약 시 : 여권정보 (여권번호, 만료일, 발급처, 국적)</p>
<p>- 항공권 결제시 : 카드번호, 유효기간, 거래 인증번호(신용카드 결제 시), 거래번호(계좌이체 거래 시)</p>
<p>- 회원 가입 및 회원정보 변경 시 : 카카오클럽 회원번호, 회원등급, 회원자격 유효기간, 국/영문 성명, 생년월일, 국적, 거주지, 성별, 연락처, 이메일, 잔여마일, 최근2년 탑승횟수 및 적립마일</p>
<p>&nbsp;</p>
<p>다. 이전되는 국가, 이전 일시, 이전 방법</p>
<p>- 예약 정보 : 독일, 예약 및 결제 완료 시, 공용 네트워크 이용 전송(보안 프로토콜 처리)</p>
<p>- 회원 정보 : 독일, 회원가입 및 회원정보 변경 시, 전용 네트워크 이용 전송</p>
<p>&nbsp;</p>
<p>라. 이전받는 자의 개인정보 이용 목적 및 보유, 이용기간</p>
<p>- 예약 정보 : 당사 예약시스템 운영 및 예약기록 유지, 개인정보 이전 후 5년 간</p>
<p>- 회원 정보 : 회원 서비스 이용 및 개인 식별 등, 계약 종료 또는 회원 탈퇴 시까지</p></div></div>
</body>
</html>